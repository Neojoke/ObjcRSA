...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    data = [DTRSA stripPublicKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (NSString *)encryptString:(NSString *)str privateKey:(NSString *)privKey{
    NSData *data = [DTRSA encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] privateKey:privKey];
...
+ (NSData *)encryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    SecKeyRef keyRef = [DTRSA addPrivateKey:privKey];
    if(!keyRef){
        return nil;
    }
    return [DTRSA encryptData:data withKeyRef:keyRef isSign:YES];
}
...
+ (NSString *)decryptString:(NSString *)str privateKey:(NSString *)privKey{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [DTRSA decryptData:data privateKey:privKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
...
+ (NSData *)decryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    return [DTRSA decryptData:data withKeyRef:keyRef];
}

/* END: Encryption & Decryption with DTRSA private key */

/* START: Encryption & Decryption with DTRSA public key */

+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
    NSData *data = [DTRSA encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey];
    NSString *ret = base64_encode_data(data);
    return ret;
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
    ...
    SecKeyRef keyRef = [DTRSA addPublicKey:pubKey];
    if(!keyRef){
        return nil;
    }
    return [DTRSA encryptData:data withKeyRef:keyRef isSign:NO];
}

+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [DTRSA decryptData:data publicKey:pubKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
